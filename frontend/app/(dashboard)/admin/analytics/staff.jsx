...
export default function StaffAnalytics() {
  ...
  const fetch = async () => {
    ...
    try {
      const resp = await AdminAPI.getStaffAnalytics(filters);
      if (resp.success) {
    ...
    <div className="p-6 space-y-6">
      <SectionWrapper>
        <div className="flex flex-wrap gap-4">
          <StatsCard title="Total staff" value={stats.total} accent="orange" />
          <StatsCard title="Active staff" value={stats.active} />
        </div>
      </SectionWrapper>
      ...
      <SectionWrapper title="Charts">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ChartCard title="Order completion rates" />
        </div>
      </SectionWrapper>

      <SectionWrapper title="Staff list">
