...
export default function RevenueAnalytics() {
  ...
  const fetch = async () => {
    ...
    try {
      const resp = await OrdersAPI.getRevenueStats(filters);
      if (resp.success) {
    ...
    <div className="p-6 space-y-6">
      <SectionWrapper>
        <div className="flex flex-wrap gap-4">
          <StatsCard title="Total revenue" value={`$${Number(stats.total||0).toFixed(2)}`} accent="orange" />
          <StatsCard title="Outstanding" value={`$${Number(stats.outstanding||0).toFixed(2)}`} />
        </div>
      </SectionWrapper>
      ...
      <SectionWrapper title="Charts">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ChartCard title="Vendor contributions" />
        </div>
      </SectionWrapper>
