...
export default function AddBranchModal({ isOpen, onClose, vendorId, onAdded }) {
  ...
  const handleAdd = async () => {
    console.log("🟠 [AddBranchModal] CREATE BUTTON CLICKED");
    setError(null);

    console.log('📦 [AddBranchModal] handleAdd called, vendorId:', vendorId);

    // Validation - only name, city, and coordinates are absolutely required
    if (!vendorId) {
      return setError('Vendor ID is missing');
    }
    ...
    // Check if required fields for active status are present
    const missingRequired = [];
    if (!form.phone?.trim()) missingRequired.push('Phone');
    if (!form.email?.trim()) missingRequired.push('Email');
    if (!form.manager?.trim()) missingRequired.push('Manager Name');
    
    const willBePending = missingRequired.length > 0;
    ...
    try {
      ...
      console.log('📦 [AddBranchModal] Creating branch with vendorId:', vendorId);
      console.log('📦 [AddBranchModal] Payload:', payload);
      
      if (willBePending) {
        console.log('⚠️ [AddBranchModal] Branch will be PENDING due to missing:', missingRequired.join(', '));
      }

      const resp = await VendorsAPI.createBranch(vendorId, payload);

      console.log('📦 [AddBranchModal] Response:', resp);

      if (resp.success || resp.data || resp.message === 'Branch created successfully') {
        console.log('✅ [AddBranchModal] Branch created successfully, calling onAdded');
        
        // Wait for parent to refresh before closing
        if (onAdded) {
          await onAdded(resp.data);
        }
          ...
          <div className="space-y-4 pb-4 border-b border-gray-200">
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800 font-medium">
                ℹ️ Required for Active Status: Manager Name, Phone, and Email
              </p>
              <p className="text-xs text-blue-600 mt-1">
                Branches without these details will be marked as <strong>Pending</strong> and hidden from customers.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input
                label="Manager Name (Required for Active) *"
                value={form.manager}
                onChange={(e) => setForm({ ...form, manager: e.target.value })}
                placeholder="Branch manager name"
              />
              <Input
                label="Phone (Required for Active) *"
                type="tel"
                value={form.phone}
            ...
            <Input
              label="Email (Required for Active) *"
              type="email"
              value={form.email}
